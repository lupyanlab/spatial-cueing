...
## Plot by-subject accuracies

1. Compute by-subject accuracies
```{r, summarize-data-for-plot}
cueing_by_subj <- cueing %>% group_by(subj_id, cue_type) %>%
  summarize(accuracy = mean(is_correct))
```

2. Arrange cue types on the x-axis
```{r, place-cueing-variables-on-x-axis}
source("R/recoders.R")
...
3. Plot of subject accuracies by cue type
```{r, "plot-of-subj-accuracies-by-cue", message = FALSE, echo = -4}
library(ggplot2)
library(scales)
...
plot_bysubj <- ggplot(cueing_by_subj,
...
Note the three outliers.
```{r}
cueing_by_subj %>% filter(accuracy[cue_type == "nocue"] < 0.7)
```

## Fit glmer model to raw accuracies

1. Set the contrast codes
```{r, "set-contrasts"}
source("R/recoders.R")
cueing <- set_treatment_contrasts(cueing)
contrasts(cueing$cue_type)
```

2. Predict likelihood of error from cue type
- leaving the outlier subjects in for now
- errors are false alarms and misses
```{r, "fit-accuracy-mod", message = FALSE}
library(lme4)
...
## Add model predictions to by-subject plot
...
x_preds$cue_type_int <- recode_cue_type_as_int(x_preds)
...
2. By subject averages with model predictions overlayed
```{r, "plot-with-model-predictions", echo = -2}
plot_accuracy_mod <- plot_bysubj +
